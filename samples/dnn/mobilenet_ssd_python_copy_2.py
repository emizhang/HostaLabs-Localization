            ...
            if confidence > args.thr:
                ...
                if class_id in classNames:
                    ...
                    if confidence > best_confidence.get(l, 0):
                        ...
                        try:
                            output[l].append([best_LeftBottom_pt, best_width, best_height, best_confidence[l]])
                        except KeyError:
                            output[l] = [[best_LeftBottom_pt, best_width, best_height, best_confidence[l]]]
                    ...
                    cv.rectangle(frame, (xLeftBottom, yLeftBottom - labelSize[1]*2),
                                         (xLeftBottom + labelSize[0]*3, yLeftBottom + baseLine*2),
...
for label, info in output.items():
    ...
    for ind, i in enumerate(info[0]):
        if ind == 3:  # Don't want to round confidence score since it is a float between 0 and 1
            break
        if type(i) == tuple:
            output[label][0][ind] = (int(i[0]), int(i[1]))
        else:
            output[label][0][ind] = int(i)
    ...
    cv.rectangle(original_img, (bottomleftpt[0], bottomleftpt[1] - labelSize[1]*2), (bottomleftpt[0] + labelSize[0]*3, bottomleftpt[1] + baseLine*2), (255, 255, 255), cv.FILLED)
    cv.putText(original_img, l, bottomleftpt, cv.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 0), 3)
    
    cv.rectangle(original_img, bottomleftpt, (bottomleftpt[0] + width, bottomleftpt[1] + height), (0, 255, 0), 4)
